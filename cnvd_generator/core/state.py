...
class StateManager:
    ...
    def normalize_report_name(report_name: str) -> str:
        """Normalize report name to a stable workspace-safe directory name."""
        raw_name = str(report_name or "").strip()
        safe_name = "".join(c for c in raw_name if c.isalnum() or c in ("-", "_"))
        return safe_name or "task"

    def get_task_dir(self, report_name: str) -> Path:
        """Return canonical task directory path for a report name."""
        return self.workspace_dir / self.normalize_report_name(report_name)

    def _get_task_dir(self, report_name: str) -> Path:
        """Get task directory path (canonical)."""
        return self.get_task_dir(report_name)
    ...
    def load_state(self, report_name: str) -> Optional[TaskState]:
        ...
        if not state_file.exists():
            legacy_state_file = self.workspace_dir / str(report_name) / "state.json"

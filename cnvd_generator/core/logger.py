...
class AgentLogger:
    ...
    def __init__(self, agent_name: str, task_id: str, log_dir: str = "workspace/logs"):
        ...
        if not self.logger.handlers:
            ...
            task_log_dir = Path(f"workspace/{task_id}/logs")
    ...
    def log_error_with_context(self, error: Exception,
        ...
        # 记录到专门的error日志文件
        error_log_path = self.log_dir / f"{self.task_id}_errors.json"
    ...
    def get_logs(self, level: str = None, tail: int = None) -> list:
        """获取日志内容"""
        log_file = Path(f"workspace/{self.task_id}/logs/{self.agent_name}.log")
